...
variable "irsa_tag_key" {
  ...
  default = "karpenter.sh/provisioner-name"
}
...
variable "create_access_entry" {
  description = "Whether to create an EKS access entry"
  type        = bool
  default     = true
}

variable "eks_access_entry_tags" {
  description = "A map of additional tags to add to the EKS access entry"
  type        = map(string)
  default     = {}
}
