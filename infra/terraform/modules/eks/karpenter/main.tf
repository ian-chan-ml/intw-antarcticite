...
locals {
  ...
  irsa_name        = coalesce(var.irsa_name, "KarpenterControllerRoleIRSA-${var.cluster_name}")
...
resource "aws_iam_policy" "policy" {
  name        = "Karpenter-EC2-Access-${var.cluster_name}"
  path        = "/"
  description = "Access to create/delete ebs volumes"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
data "aws_iam_policy_document" "irsa_v2" {
  count = local.create_irsa ? 1 : 0
  statement {
    sid = "Karpenter"
    actions = [
...
resource "aws_iam_policy" "irsa" {
  count = local.create_irsa ? 1 : 0
  name  = local.irsa_policy_name
  #   name_prefix = "${local.irsa_policy_name}-"
  path        = var.irsa_path
  description = var.irsa_description
  policy      = data.aws_iam_policy_document.irsa_v2[0].json
...
locals {
  ...
  iam_role_name          = coalesce(var.iam_role_name, "KarpenterNodeRole-${var.cluster_name}")
...
resource "aws_iam_role_policy_attachment" "this" {
  for_each = { for k, v in toset(compact([
    ...
    "arn:aws:iam::${local.account_id}:policy/Karpenter-EC2-Access-${var.cluster_name}",
...
resource "aws_eks_access_entry" "this" {
  count = var.create && var.create_access_entry && var.create_iam_role ? 1 : 0

  cluster_name  = var.cluster_name
  principal_arn = aws_iam_role.this[0].arn
  type          = "EC2_LINUX"

  tags = merge(var.tags, var.eks_access_entry_tags)
}
